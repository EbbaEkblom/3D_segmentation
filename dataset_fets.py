...
class BrainTumour3DSegmentation(Dataset):
    """
    ...
    def __init__(self, dir_path, split_ratios = [0.8, 0.1, 0.1], transforms = None, mode = None) -> None:
        super(BrainTumour3DSegmentation, self).__init__()
        ...
        num_training_imgs =  1254 # self.meta["numTraining"]
    ...
    def __getitem__(self, idx):
        ...
        label_object = nib.load(img_path + '/' + name + '_seg.nii')
        label_array = label_object.get_fdata()
        
        channels = ['flair', 't1', 't1ce', 't2']
        img_array = np.zeros([label_array.shape[0], label_array.shape[1], label_array.shape[2], len(channels)])
        for c in range(len(channels)):
            img_object = nib.load(img_path + '/' + name + '_' + channels[c] + '.nii')
            im = img_object.get_fdata()
            img_array[:,:,:,c] = im
...
def get_train_val_test_Dataloaders(train_transforms, val_transforms, test_transforms):
    ...
    dataset = BrainTumour3DSegmentation(dir_path=DATASET_PATH, split_ratios=TRAIN_VAL_TEST_SPLIT, transforms=[train_transforms, val_transforms, test_transforms])
